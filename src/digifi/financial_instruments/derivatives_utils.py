...
class OptionPayoffs:
    ...
    @staticmethod
    def bear_spread(s_t: np.ndarray, k_c: float=0.0, k_p: float=0.0) -> np.ndarray:
        """
        ## Description
        ...
        """
        type_check(value=s_t, type_=np.ndarray, value_name="s_t")
        return np.maximum(float(k_c)-s_t, np.zeros(len(s_t))) + np.minimum(s_t-float(k_p), np.zeros(len(s_t)))
