...
class OptionPayoffs:
    ...
    def bear_collar_payoff(s_t: np.ndarray, k_p: float=0.0, k_c: float=0.0) -> np.ndarray:
        ...
        ### Input:
            - s_t: Underlying asset price
            - k_p: Long call option strike price
            - k_c: short put option strike price
        ### Output:
            - Payoff of the bear collar option
        ### LaTeX Formula:
    ...
    def long_butterfly(s_t: np.ndarray, k_1: float=0.0, k: float=0.0, k_2: float=0.0) -> np.ndarray:
        ...
        ### Input:
            - s_t: Underlying asset price
            - k_1: Smaller long call option strike price
            - k: Short put option strike price
            - k_2: Larger long call option strike price
        ### Output:
    ...
    def straddle(s_t: np.ndarray, k: float=0.0) -> np.ndarray:
        ...
        ### Input:
            - s_t: Underlying asset price
            - k: Straddle option strike price
        ### Output:
    ...
    def strangle(s_t: np.ndarray, k_1: float=0.0, k_2: float=0.0) -> np.ndarray:
        ...
        ### Input:
            - s_t: Underlying asset price
            - k_1: Smaller stangle option strike price
            - k_2: Larger strangle option strike price
        ### Output:
...
def validate_custom_payoff(custom_payoff: Type[CustomPayoff], length_value: int=5) -> Type[CustomPayoff]:
    ...
    ### Input:
        - custom_payoff: Custom payoff class to be validated
        - length_value: Number of test data points to validate payoff method on
    ### Output:
        - Validated custom payoff class
    """
