...
class BondStruct(DataClassValidation):
    ...
    ### Input:
        - bond_type: Type of bond (i.e., ANNUITY_BOND (Coupon bond), GROWING_ANNUITY_BOND (Coupon bond with growing coupon), ZERO_COUPON_BOND, or CUSTOM)
        - principal: Principal of the bond
        - coupon_rate: Coupon rate of the bond (Initial coupon rate for the GROWING_ANNUITY_BOND)
        ...
        - compounding_frequency: Compounding frequency of bond's payoff (e.g., for a bond with 3% semi-annual coupon rate, the compounding frequency is 2)
        - coupon_growth_rate: Growth rate of the coupon
        - inflation_rate: Inflation rate with which the cashflows will be discounted (Applies multiplicatively on top of discoun_rates)
        - first_coupon_time: Time at which the initial coupon is scheduled to be paid
    """
    bond_type: BondType
    principal: float
    coupon_rate: float
...
def bond_price_from_yield(current_price: float, duration: float, convexity: float, yield_spread: float) -> float:
    ...
    ### Input:
        ...
        - duration: Duration of the bond (i.e., first derivative of bond price with respect to yield)
        - convexity: Convexity of the bond (i.e., second derivative of bond price with respect to yield)
...
def bootstrap(principals: np.ndarray, maturities: np.ndarray, coupons: np.ndarray, prices: np.ndarray, coupon_dt: np.ndarray) -> np.ndarray:
    ...
    ### Input:
        ...
        - coupons: Array of bond coupons (Coupns are assumed to remain constant with respect to future payoffs)
        - prices: Array of current market prices of bonds
        coupon_dt: Array of differences between times of coupon payments (e.g., for semi-annual coupon coupon_dt=0.5)
    ### Output:
...
class Bond(FinancialInstrumentInterface, BondInterface):
    ...
    ### Input:
        - bond_struct: Parameters for defining a Bond instance
    ...
    def yield_to_maturity(self, ytm_method: YtMMethod=YtMMethod.NUMERICAL) -> float:
        ...
        ### Input:
            - ytm_method: Method for evaluating yield-to-maturity (i.e., NUMERIACAL - for numerical solution of the IRR, APPROXIMATION - for approximation formula)
        ### Output:
